...
Simulate creates isolated, resettable environments to test, investigate, and evaluate AI
agents against realistic business workflows. It provides deterministic sandboxes, synthetic
user simulation, privacy-safe bundles, repeated experiments, and cloud investigation runners.
Simulate reports objective evidence; humans make release and production decisions.
...
|:---|:---|:---|
| [`src/`](src/codemap.md) | Source package for all application code. | [View Map](src/codemap.md) |
...
| [`src/app/adapters/sources/`](src/app/adapters/sources/codemap.md) | Trace and feedback source adapters. | [View Map](src/app/adapters/sources/codemap.md) |
...
| [`src/app/domain/`](src/app/domain/codemap.md) | Domain services, models, policies, and state machines. | [View Map](src/app/domain/codemap.md) |
...
| [`src/app/domain/comparison/`](src/app/domain/comparison/codemap.md) | Baseline/candidate comparisons and evaluators. | [View Map](src/app/domain/comparison/codemap.md) |
...
| [`src/app/domain/reference/`](src/app/domain/reference/codemap.md) | Reference workflow contracts, runners, comparisons, and reports. | [View Map](src/app/domain/reference/codemap.md) |
| [`src/app/domain/reference/workflows/`](src/app/domain/reference/workflows/codemap.md) | Reference workflow implementations and fixture sets. | [View Map](src/app/domain/reference/workflows/codemap.md) |
...
| [`src/app/domain/reference_workflows/ci_triage/`](src/app/domain/reference_workflows/ci_triage/codemap.md) | CI-triage reference fixtures. | [View Map](src/app/domain/reference_workflows/ci_triage/codemap.md) |
| [`src/app/domain/reference_workflows/claims_denial/`](src/app/domain/reference_workflows/claims_denial/codemap.md) | Claims-denial reference fixtures. | [View Map](src/app/domain/reference_workflows/claims_denial/codemap.md) |
| [`src/app/domain/reference_workflows/incident_response/`](src/app/domain/reference_workflows/incident_response/codemap.md) | Incident-response reference fixtures. | [View Map](src/app/domain/reference_workflows/incident_response/codemap.md) |
...
| [`alembic/versions/`](alembic/versions/codemap.md) | Ordered database revision history. | [View Map](alembic/versions/codemap.md) |

## End-to-End Flows

1. **Evidence to case:** trace adapter -> evidence import -> deterministic failure features and
   groups -> human review -> privacy allowlist -> approved simulation bundle.
2. **Case to result:** saved case or suite -> execution service -> isolated PostgreSQL sandbox
   -> support agent and evaluators -> allowlisted event stream -> rollback -> immutable result.
3. **Baseline to candidate:** identical bundle and seed -> interleaved repeated runs -> identity,
   measurement, and safety gates -> neutral comparison verdict.
4. **Workflow:** API request -> LangGraph state -> retrieval and policy nodes -> actor-bound human
   confirmation -> execute, reject, or escalate -> persisted checkpoint state.
5. **User simulation:** YAML manifest -> validation and preflight -> registered flow plugin ->
   multi-turn user/support exchange -> privacy-separated events and report -> cleanup.
6. **Investigation:** CLI/API start -> investigation service and task brief -> Modal or fake runner
   -> sandbox bridge, Prime RPC, and World Gateway -> token-authenticated event ingestion -> SSE
   replay and terminal sandbox cleanup.
